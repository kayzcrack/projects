...
-- generates identification number for CPCD.
DELIMITER //

DROP FUNCTION IF EXISTS `makeCDIN` //
CREATE FUNCTION `makeCDIN`(in_Surname VARCHAR(32)) RETURNS CHAR(15) CHARSET latin1
DETERMINISTIC
BEGIN
SET @prefix = 'CPCD/';
SET @ldin=(SELECT CDIN FROM CDINs WHERE DateUsed IS NULL LIMIT 1);
UPDATE CDINs SET DateUsed=CURRENT_DATE() WHERE CDIN =@ldin; 
