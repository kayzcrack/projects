...
DELIMITER ;
-- =====================================
-- Returns all Selector from the database.
DELIMITER //
DROP PROCEDURE IF EXISTS getSelector //
CREATE PROCEDURE getSelector()
BEGIN
 SELECT SelectorID, Description FROM Selector;
END //
DELIMITER ;
...
-- add AULevel1ID in where clause
DELIMITER //
...
-- add AULeve1 and AULevel2 ID's to parameter list
DELIMITER //
...
BEGIN
  ...
  SET @strDate = '2000-01-01 10:33:10';
  SET @endDate = '2020-01-01 10:33:10';
  ...
  IF in_StartDate = "0000-00-00 00:00:00" AND in_EndDate = "0000-00-00 00:00:00" THEN 
		  ...
		  LEFT JOIN AULevel3 B ON A.CountryCode = B.CountryCode AND A.AULevel3ID = B.AULevel3ID AND A.AULevel2ID = B.AULevel2ID
                       AND A.AULevel1ID = B.AULevel1ID
		  LEFT JOIN AULevel2 C ON A.AULevel2ID = C.AULevel2ID AND A.AULevel1ID = C.AULevel1ID
                       AND A.CountryCode = C.CountryCode
		  LEFT JOIN AULevel1 D ON A.AULevel1ID = D.AULevel1ID AND A.CountryCode = D.CountryCode
		  LEFT JOIN Country E ON A.CountryCode = E.CountryCode
		  WHERE A.TeamID = in_TeamID AND A.StartDate BETWEEN @strDate AND @endDate;
ELSE
		  ...
		  LEFT JOIN AULevel1 D ON A.AULevel1ID = D.AULevel1ID AND A.CountryCode = D.CountryCode
		  LEFT JOIN Country E ON A.CountryCode = E.CountryCode
		  WHERE A.TeamID = in_TeamID AND A.StartDate BETWEEN in_StartDate AND @endDate;
		...
		ELSE
		  ...
		  LEFT JOIN AULevel1 D ON A.AULevel1ID = D.AULevel1ID AND A.CountryCode = D.CountryCode
		  LEFT JOIN Country E ON A.CountryCode = E.CountryCode
		  WHERE A.TeamID = in_TeamID AND A.StartDate BETWEEN @strDate AND in_EndDate;

              
		  ELSE IF in_StartDate <> "" AND in_EndDate <> "" THEN 
                  SELECT A.AULevel3ID,B.Name,A.CountryCode,C.Name,A.AULevel2ID,D.Name,A.AULevel1ID,E.Name
		  FROM TeamAreaOfCoverage A
		  LEFT JOIN AULevel3 B ON A.CountryCode = B.CountryCode AND A.AULevel3ID = B.AULevel3ID AND A.AULevel2ID = B.AULevel2ID
                       AND A.AULevel1ID = B.AULevel1ID
		  ...
		  LEFT JOIN AULevel1 D ON A.AULevel1ID = D.AULevel1ID AND A.CountryCode = D.CountryCode
		  LEFT JOIN Country E ON A.CountryCode = E.CountryCode
                  WHERE A.TeamID = in_TeamID AND A.StartDate BETWEEN in_StartDate AND in_EndDate;
...
CREATE PROCEDURE getAddress(IN in_FacilityID INT, IN in_HumanID INT)  
BEGIN
...
SET @Hum = (SELECT RespondentID FROm Respondent WHERE HumanID = in_HumanID);
  ...
  /*IF in_FacilityID = "" AND in_HumanID = "" THEN
	  SELECT A.AddressTypeID, B.Description,A.CountryCode,C.Name,A.AULevel1ID,D.Name,A.AULevel2ID,
              E.Name, A.AULevel3ID,F.Name,A.Addressline1,A.Addressline2
	  FROM Address A
  ...
  IF in_FacilityID <> "" AND in_HumanID = "" THEN
	  ...
	  FROM Address A
	  LEFT JOIN AddressType B ON A.AddressTypeID = B.AddressTypeID
	  LEFT JOIN AULevel3 F ON A.CountryCode = F.CountryCode AND A.AULevel1ID = F.AULevel1ID AND A.AULevel2ID = F.AULevel2ID AND A.AULevel3ID = F.AULevel3ID
	  LEFT JOIN AULevel2 E ON F.CountryCode = E.CountryCode AND F.AULevel1ID = E.AULevel1ID AND F.AULevel2ID = E.AULevel2ID
	  LEFT JOIN AULevel1 D ON E.CountryCode = D.CountryCode AND E.AULevel1ID = D.AULevel1ID
	  LEFT JOIN Country C ON D.CountryCode = C.CountryCode
      WHERE A.RespondentID = @Fac;
  ELSE
...
-- ===============================================================
-- returns all schedule question for active indicator from the database for specified period of time
	DELIMITER //
	DROP PROCEDURE IF EXISTS getScheduleActiveIndicator //
	CREATE PROCEDURE getScheduleActiveIndicator(IN in_StartDate DATE, IN in_EndDate DATE)  
	BEGIN 
                  SET @strDate = '2000-01-01';
                  SET @endDate = '2020-01-01';

                    IF in_StartDate = "0000-00-00" AND in_EndDate = "0000-00-00" THEN
		        SELECT A.ScheduleID,B.IndicatorClassID,C.Description,D.IndicatorID,B.Name,A.QuestionID,
			D.Question,A.StatusID,E.Description,A.FrequencyID,F.Description,A.MethodMeasureID,G.Description
			FROM ScheduledActiveIndicator A
			LEFT JOIN Question D ON A.QuestionID = D.QuestionID
			LEFT JOIN FrequencyOfMeasure  F ON A.FrequencyID = F.FrequencyID
			LEFT JOIN MethodOfMeasurement G ON A.MethodMeasureID = G.MethodMeasureID
			LEFT JOIN Indicator B ON B.IndicatorID = D.IndicatorID
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StartDATE BETWEEN @strDate AND @endDate;
                  ELSE
                  
                  IF in_StartDate <> "" AND in_EndDate = "0000-00-00" THEN
		        SELECT A.ScheduleID,B.IndicatorClassID,C.Description,D.IndicatorID,B.Name,A.QuestionID,
			D.Question,A.StatusID,E.Description,A.FrequencyID,F.Description,A.MethodMeasureID,G.Description
			FROM ScheduledActiveIndicator A
			LEFT JOIN Question D ON A.QuestionID = D.QuestionID
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StartDATE BETWEEN in_StartDate AND @endDate;
                 
                  ELSE

                  IF in_StartDate = "0000-00-00" AND in_EndDate <> "" THEN
		        SELECT A.ScheduleID,B.IndicatorClassID,C.Description,D.IndicatorID,B.Name,A.QuestionID,
			D.Question,A.StatusID,E.Description,A.FrequencyID,F.Description,A.MethodMeasureID,G.Description
			FROM ScheduledActiveIndicator A
			LEFT JOIN Question D ON A.QuestionID = D.QuestionID
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StartDATE BETWEEN @strDate AND in_EndDate;
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StartDATE BETWEEN in_StartDate AND in_EndDate;
                 END IF;
                       END IF;
                              END IF; 
                                     END IF;
...
-- =================================================================
-- returns scheduled indicator for a specific status.
DELIMITER //
	DROP PROCEDURE IF EXISTS getScheduleActiveIndicatorStatus //
	CREATE PROCEDURE getScheduleActiveIndicatorStatus(IN in_StatusID CHAR(3),IN in_StartDate DATE, IN in_EndDate DATE)  
	BEGIN 
                  SET @strDate = '2000-01-01';
                  SET @endDate = '2020-01-01';
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StatusID = in_StatusID AND A.StartDATE BETWEEN @strDate AND @endDate;
                  ELSE
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StatusID = in_StatusID AND A.StartDATE BETWEEN in_StartDate AND @endDate;
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StatusID = in_StatusID AND A.StartDATE BETWEEN @strDate AND in_EndDate;
			...
			LEFT JOIN IndicatorClass C ON B.IndicatorClassID = C.IndicatorClassID
			LEFT JOIN IndicatorStatus E ON B.StatusID = E.StatusID
                        WHERE A.StatusID = in_StatusID AND A.StartDATE BETWEEN in_StartDate AND in_EndDate;
                 END IF;
                       END IF;
                              END IF; 
                                     END IF;
