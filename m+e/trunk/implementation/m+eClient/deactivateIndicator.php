...
//	$teamId = $_POST['tid'];
	...
	$url = "http://localhost/m+eWebService/deactivateIndicator";
		$post = "indicatorid=$indicatorid&username=$username&permission=$permission";
			$handle = curl_init();
			curl_setopt_array(
			$handle,
			array(
    ...
    <td><form id="form1" name="cuser" method="post" action="" onsubmit="return validateForm()">
      <table width="100%">
        ...
        <tr>
          ...
          <td>indicatorcid</td>
          <td>
            <input type="text" name="iid" id="textfield" tabindex="5" />
