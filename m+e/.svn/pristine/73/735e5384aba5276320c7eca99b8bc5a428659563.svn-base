...
<?php  
	...
	$url = "http://localhost/m+eWebService/addAUL2";
		$post = "countrycode=$countryCode&aul1id=$aul1Id&aul2id=$aul2Id&name=$name&username=$username&permission=$permission";
			$handle = curl_init();
			curl_setopt_array(
			$handle,
			array(
...
$.post('http://localhost/m+eWebService/getAllAULevel1', function(data){
        ...
        <tr>
          <td>&nbsp;</td>
          <td>au level 2 id</td>
          <td><input type="text" name="aul2id" id="textfield" tabindex="15" />
