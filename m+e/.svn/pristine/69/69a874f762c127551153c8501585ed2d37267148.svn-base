...
<?php  
	...
	$url = "http://localhost/m+eWebService/addAUL1";
		$post = "ccode=$countryCode&aul1id=$aul1id&name=$name&username=$username&permission=$permission";
			$handle = curl_init();
			curl_setopt_array(
			$handle,
			array(
			...
			try{
					...
					$json = json_decode($string, true);
					  if($json['status'] == 200)
					  {
						  ...
						  $msg = "AU Level 1 added successfully";
...
?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="logic/logout.php">sign out</a>
      ...
      <h3>Add AU Level 1</h3>  <h6>Note: fields with * are required.</h6></div></td>
    </tr>
  <tr>
...
$.post('http://localhost/m+eWebService/getCountries', function(data){
        ...
        <tr>
          ...
          <td><strong class="asterix">
            <input type="text" name="aul1id" id="textfield" tabindex="10" />
            * </strong></td>
          <td>&nbsp;</td>
        </tr>
        <tr>
