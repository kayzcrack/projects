...
-- CREATE PROCEDURE loginUser(IN in_Username,IN in_Password)
--	BEGIN 
...
-- CREATE PROCEDURE logoutUser(IN in_Username,IN in_Password)
-- 	BEGIN 
...
CREATE PROCEDURE addVideo(IN in_VideoTitle varchar(64),IN in_AuthorID int,IN in_CategoryID char(5),IN in_Size varchar(8),IN in_Duration Time,IN in_FormatID char(3),IN in_SubCategoryID char(8),IN in_Rating Double)
BEGIN 
		SET @videoid = makeSDIN(in_VideoTitle);
		INSERT INTO Video(VideoID,VideoTitle,AuthorID,CategoryID,Size,Duration,FormatID,SubCategoryID,Current_Rating)
		VALUES(@videoid,in_VideoTitle,in_AuthorID,in_CategoryID,in_Size,in_Duration,in_FormatID,
                       in_SubCategoryID,in_Rating);
	END ##
DELIMITER ;
-- ==================================================================
DELIMITER ##
...
CREATE PROCEDURE addVideoViewCount(IN in_VideoID varchar(16),IN in_ViewDateTime Datetime,IN in_Username Varchar(16))
	BEGIN 
		INSERT INTO VideoViewCount(VideoID,ViewDateTime,Username)
		VALUES(in_VideoID,in_ViewDateTime,in_Username);
	END ##
DELIMITER ;
-- ==================================================================
DELIMITER ##
...
create procedure getUserFavourite(IN in_Username varchar(1))
begin
 select uf.VideoID,v.VideoTitle,uf.Username, concat(u.Surname, ' ' ,u.Firstname) User
 from UserFavourite uf
 left join Video v on uf.VideoID = v.VideoID
